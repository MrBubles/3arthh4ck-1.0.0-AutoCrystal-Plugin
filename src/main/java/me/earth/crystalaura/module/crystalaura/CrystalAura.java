...
import me.earth.crystalaura.module.crystalaura.modes.*;
import me.earth.crystalaura.module.crystalaura.util.CrystalAuraData;
import me.earth.crystalaura.module.crystalaura.util.EntityTime;
import me.earth.crystalaura.module.crystalaura.util.TaskThread;
...
public class CrystalAura
    ...
    protected void setTarget(EntityPlayer target) {
    ...
    protected void runThread() {
        if (CrystalAura.mc.world != null && CrystalAura.mc.player != null && !this.isPingBypass()) {
            ArrayList<EntityPlayer> players = new ArrayList<EntityPlayer>(CrystalAura.mc.world.playerEntities);
            ArrayList<Entity> crystals = new ArrayList<Entity>(CrystalAura.mc.world.loadedEntityList);
    ...
    protected boolean shouldFacePlace() {
        return this.holdFP.getValue() && Mouse.isButtonDown(0) && !CrystalAura.mc.playerController.getIsHittingBlock() && !(CrystalAura.mc.player.getHeldItemMainhand().getItem() instanceof ItemPickaxe) && !(CrystalAura.mc.currentScreen instanceof GuiContainer);
    }
    ...
    protected void checkKilled() {
        if (CrystalAura.mc.world != null && !this.isPingBypass()) {
            for (Map.Entry<Integer, EntityTime> entry : this.killed.entrySet()) {
                ...
                entity.isDead = false;
                if (CrystalAura.mc.world.loadedEntityList.contains(entity)) continue;
                CrystalAura.mc.world.addEntityToWorld(entry.getKey().intValue(), entity);
                entity.isDead = false;
                this.killed.remove(entry.getKey());
            }
        }
