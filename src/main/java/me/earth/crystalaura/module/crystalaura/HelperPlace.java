...
import me.earth.crystalaura.module.crystalaura.modes.Target;
import me.earth.crystalaura.module.crystalaura.util.PlaceData;
...
final class HelperPlace
    ...
    private boolean isValid(EntityPlayer player, BlockPos pos) {
        if (player != null && !EntityUtil.isDead(player) && !player.equals(HelperPlace.mc.player)) {
            return player.getDistanceSq(pos) <= (double) MathUtil.square(this.value.range.getValue().floatValue());
        }
        return false;
    }
    ...
    private boolean isValid(PlaceData data, BlockPos pos) {
        if (BlockUtil.getDistanceSq(pos) <= (double) MathUtil.square(this.value.placeRange.getValue().floatValue()) && BlockUtil.canPlaceCrystal(pos, this.value.antiSurr.getValue(), this.value.newerVer.getValue(), this.getEntityList(data))) {
            if (HelperPlace.mc.player.getDistanceSq(pos) > (double) MathUtil.square(this.value.placeTrace.getValue().floatValue()) && !RayTraceUtil.raytracePlaceCheck(HelperPlace.mc.player, pos)) {
                return false;
            }
    ...
    private EntityPlayer getPlayer(EntityPlayer player) {
    ...
    private EntityPlayer interpolate(EntityPlayer player) {
        Vec3d last = new Vec3d(player.lastTickPosX, player.lastTickPosY, player.lastTickPosZ);
        Vec3d current = player.getPositionVector();
        Vec3d diff = last.subtract(current);
        if (diff.lengthSquared() < 0.001) {
            return player;
        }
        EntityOtherPlayerMP out = new EntityOtherPlayerMP(HelperPlace.mc.world, player.getGameProfile());
        ...
        for (int i = 0; i < this.value.interpolate.getValue() && HelperPlace.mc.world.getBlockState(new BlockPos(current.add(diff))).getMaterial().isReplaceable(); ++i) {
