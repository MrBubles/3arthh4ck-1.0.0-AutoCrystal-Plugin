...
import me.earth.crystalaura.module.crystalaura.util.PlayerUtil;
    ...
    public void invoke(MotionUpdateEvent event) {
        ...
        if (event.getStage() == Stage.PRE) {
            if (this.module.flooder.getValue().booleanValue()) {
                if (this.module.floodService == null && InventoryUtil.isHolding(Items.END_CRYSTAL) && EntityUtil.getClosestEnemy() != null && PlayerUtil.isInHole(EntityUtil.getClosestEnemy()) && PlayerUtil.isFeetPlaceable(EntityUtil.getClosestEnemy(), true, false, this.module.placeRange.getValue().floatValue()) && !BOMBER.isEnabled()) {
                ...
                } else if (this.module.floodService != null && this.module.currentRunnable != null && (ListenerMotion.mc.player.getDistanceSq(this.module.currentRunnable.getTarget()) >= (double) MathUtil.square(this.module.targetRange.getValue().floatValue()) || ListenerMotion.mc.player.getDistanceSq(this.module.currentRunnable.getPos()) >= (double) MathUtil.square(this.module.placeRange.getValue().floatValue()) || !BlockUtil.canPlaceCrystalFuture(this.module.currentRunnable.getPos(), true, false) || !PlayerUtil.isFootPlace(this.module.currentRunnable.getPos(), this.module.currentRunnable.getTarget(), true, false, this.module.placeRange.getValue().floatValue()) || DamageUtil.calculate(this.module.currentRunnable.getPos()) >= this.module.maxSelfP.getValue().floatValue() || !InventoryUtil.isHolding(Items.END_CRYSTAL))) {
            ...
            if (!this.module.multiThread.getValue().booleanValue()) {
                calc = new Calculation(this.module, ListenerMotion.mc.world.playerEntities, ListenerMotion.mc.world.loadedEntityList);
            ...
            if (!InventoryUtil.isHolding(Items.END_CRYSTAL) || this.module.renderTimer.passed(300L)) {
